...
{
    internal sealed class GetRandomTopicWithQuestionsQueryHandler : IRequestHandler<GetRandomTopicWithQuestionsQuery, Result<TopicWithQuestionsResponse>>
    {
        ...
        {
            var randomTopic = await _topicRepository.GetRandomRecordAsync(cancellationToken, 
                t => t.Questions);

            return randomTopic is null
               ? Result.Failure<TopicWithQuestionsResponse>(TopicErrors.NotFound)
               : _topicMapper.Map(randomTopic);
        }
    }
}
